...
//const IPAddress baseIP(192, 168, 43, 131); //IP del raspberry!!! Odile: 192.168.43.131 HONOR 9 LITE
const IPAddress baseIP(192, 168, 0, 100); //CAMBIO CON IP PC
//const IPAddress baseIP(192, 168, 43, 4); //IP pc su Honor 9 Lite

IPAddress destinationIPs [] = {baseIP};

const int raspPort = 40616;  // my mac os udp port is: 49242 | 44444 | oppure 39530

const unsigned int min_send_frequency = 100;  // how many milliseconds MUST elapse AT LEAST between two consecutive calls to SEND

int led = 0;
int buttonState = 0;
int buttonPin = 33; //22 right blue, wrong red | 27-right green | 33-wrong green yellow black, right yellow
int ledbtnPin = 5; //pin 5-right green
int led_blue_Pin = 26;  //26 - wrong green yellow black, right yellow | 14 - right blue, wrong red
int led_green_Pin = 27; //27 - wrong green yellow black, right yellow | 12 - right blue, wrong red
int led_red_Pin = 12; //12 - wrong green yellow black, right yellow | 13 - right blue, wrong red
...
class EspChannel {
    ...
    protected: 
        ...
        void readFromRobot() {
            ...
            led  = _networkChannel.read_udp_non_blocking();
    ...
    // ______________________________________________________________________________________________ SERIAL COMM END
        ...
        void loop() {
            ...
            // 2. loop sensor: get readings from sensor - DECOMMENTARE
            loop_sensor();
            
            // 3. loop send: check if it's time to SEND, then send from ALL ACTIVE ESP VALUES all the AVAILABLE DATA - DECOMMENTARE
            //loop_send();
        }
};
...
class AccEspChannel: public EspChannel {
    ...
    public: 
        ...
        AccEspChannel(IPAddress staticIP, int pinA, int pinB): EspChannel(staticIP), 
            _roll("ax", 0.5f), _pitch("ay", 0.5f), _yaw("az", 0.5f) {
