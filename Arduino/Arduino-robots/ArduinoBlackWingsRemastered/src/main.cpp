...
#define MAX_WING_ANGLE_OPEN 110
#define MIN_WING_ANGLE_OPEN 10
#define MID_WING_ANGLE_OPEN (MAX_WING_ANGLE_OPEN - MIN_WING_ANGLE_OPEN) / 2
#define MIN_WING_ANGLE_ROTATE 60
#define MAX_WING_ANGLE_ROTATE 120
#define MID_WING_ANGLE_ROTATE (MAX_WING_ANGLE_ROTATE - MIN_WING_ANGLE_ROTATE) / 2
...
//HEAD ANGLES
#define MIN_ANGLE_HEAD_BEAK_T2 0
#define MAX_ANGLE_HEAD_BEAK_T2 120
...
void reset_all_servos() {
  ...
  // wings are centered
  wings_pos = MID_WING_ANGLE_OPEN;
  servo_wings.write(MID_WING_ANGLE_OPEN);
...
void set_busto_pos(float pos, Servo servo) {

  // pos arrives in range [-1, 1] - rescale it in [PETAL_ANGLE_OPENED, PETAL_ANGLE_CLOSED]
  pos = (((pos + 1.0f) / 2.0f) * SERVO_b)  + (float)MIN_WING_ANGLE_ROTATE;

  // Make sure the position is within the limit.
  // it could not be the case if the incoming normalized setpoint was not in [-1, 1]
  if (pos < MIN_WING_ANGLE_ROTATE) {
    pos = MIN_WING_ANGLE_ROTATE;
  } else if (pos > MAX_WING_ANGLE_ROTATE) {
    pos = MAX_WING_ANGLE_ROTATE;
  }
...
void set_wings_pos(float pos, Servo servo) {

  // pos arrives in range [-1, 1] - rescale it in [PETAL_ANGLE_OPENED, PETAL_ANGLE_CLOSED]
  pos = (((pos + 1.0f) / 2.0f) * SERVO_w)  + (float)MIN_WING_ANGLE_OPEN;

  // Make sure the position is within the limit.
  // it could not be the case if the incoming normalized setpoint was not in [-1, 1]
  if (pos < MIN_WING_ANGLE_OPEN) {
    pos = MIN_WING_ANGLE_OPEN;
  } else if (pos > MAX_WING_ANGLE_OPEN) {
    pos = MAX_WING_ANGLE_OPEN;
  }
...
void onBustoValueRcv(float newVal) {
  // pos arrives in range [-1, 1]. Saturate it to those bounds.
  busto_pos = newVal;
  if (busto_pos > 1)
    busto_pos = 1;
...
void onWingsValueRcv(float newVal) {
  // pos arrives in range [-1, 1]. Saturate it to those bounds.
  wings_pos = newVal;
  if (wings_pos > 1)
    wings_pos = 1;
...
void message_response(KeyValueMsg keyValueMsg){
  ...
  else if (key == SERVO_WINGS_KEY) {
    onWingsValueRcv(value.toFloat());
    dirty = true;
    // Serial.print(" - - WINGS VAL RCV: ");
    // Serial.println(value);
    last_command_time = millis();
    }
  else if (key == SERVO_BUSTO_KEY) {
    onBustoValueRcv(value.toFloat());
    dirty = true;
