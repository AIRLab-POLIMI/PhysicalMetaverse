...
class AccEspChannel: public EspChannel {
    ...
    public: 
        ...
        AccEspChannel(IPAddress staticIP, int pinA, int pinB): EspChannel(staticIP), 
            _roll("ax", 5.0f), _pitch("ay", 5.0f), _yaw("az", 5.0f) {
        ...
        void setup_acc() {
            ...
            if (_devStatus == 0) {
                ...
                _espValues[0].setConfig(true, baseIP); //roll
                _espValues[1].setConfig(true, baseIP); //pitch
                _espValues[2].setConfig(true, baseIP); //yaw
        ...
        void loop_acc() {
            ...
            // roll (x-axis rotation)
                ...
                if((_angles.roll < 0.5 && _angles.roll>0) || (_angles.roll > -0.5 && _angles.roll<0) ){
                    _angles.roll=0;
                }
                ...
                if((_angles.pitch < 0.5 && _angles.pitch>0) || (_angles.pitch > -0.5 && _angles.pitch<0) ){
                    _angles.pitch=0;
                }
                ...
                if((_angles.yaw < 0.5 && _angles.yaw>0) || (_angles.yaw > -0.5 && _angles.yaw<0) ){
                    _angles.yaw=0;
                }
                ...
                _angles.pitch=pitch;
                _angles.yaw=yaw;*/
                _angles.roll=_angles.roll*180/PI;
                _angles.pitch=_angles.pitch*180/PI;
                _angles.yaw=_angles.yaw*180/PI;

                _angles.roll = atan2(2*_q.y*_q.z - 2*_q.w*_q.x, 2*_q.w*_q.w + 2*_q.z*_q.z - 1)*180/PI;
                _angles.pitch = -asin(2*_q.x*_q.z + 2*_q.w*_q.y)*180/PI;;
                _angles.yaw = atan2(2*_q.x*_q.y - 2*_q.w*_q.z, 2*_q.w*_q.w + 2*_q.x*_q.x - 1)*180/PI;

                
                
                if((_angles.roll <20 && _angles.roll > -20) || _angles.roll <-160 || _angles.roll> 160){
                    _angles.roll=0;
                }
                if((_angles.pitch <20 && _angles.pitch > -20) || _angles.pitch <-60 || _angles.pitch> 60){
                    _angles.pitch=0;
                }
                if((_angles.yaw <30 && _angles.yaw > -30) || _angles.yaw <-150 || _angles.yaw> 150){
                    _angles.yaw=0;
                }
                


                _espValues[0].onNewValueReceived(_angles.roll);
                _espValues[1].onNewValueReceived(_angles.pitch);
                _espValues[2].onNewValueReceived(_angles.yaw);
