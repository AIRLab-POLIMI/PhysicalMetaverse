...
//const IPAddress baseIP(192, 168, 0, 102); //IP del raspberry!!!
...
class MpxEspChannel: public EspChannel {
    ...
    private: 
        ...
        // long duration;
        const float pressureFactor = 1; // Fattore di conversione per la pressione prima era 4.9
        const float pressureAdd = 0;
        //float _distanceCm;
        // float distanceInch;
    ...
    public: 
        ...
        MpxEspChannel(IPAddress staticIP, uint8_t pinA): EspChannel(staticIP), 
            _mpx("p", 100) {
        ...
        void loop_mpx() {
            int sensorValue = analogRead(_pinA);

            // Calcola il valore di pressione in kPa
            float pressure = (sensorValue * pressureFactor) + pressureAdd; //prima anche -2.5

                //_mpx.onNewValueReceived(pressure);
                _espValues[0].onNewValueReceived(pressure);
