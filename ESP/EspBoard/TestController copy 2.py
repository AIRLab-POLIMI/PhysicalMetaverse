    ...
    while True:
        if (datetime.now() - prev_time).total_seconds() < 0.5:
            ...
            try:
                ...
                send_str = ""
                for key in _gamepadState:
                    if(_gamepadState[key] != _prevGamepadState[key]):
                        send_str = send_str + key + ":" + format_str(_gamepadState[key]) + "_"
                        _prevGamepadState[key] = _gamepadState[key]
                #remove last char
                send_str = send_str[:-1]
