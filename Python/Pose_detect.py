...
def execute(img, src, t, connection):
    color = (0, 255, 0)
    data = preprocess(img)
...
def loop(connection, screenless_mode):
    try:

        print("start pose detect loop")
        while cap.isOpened():
            t = time.time()
            ...
            if screenless_mode:
                execute(img, dst, t, connection)
            else:
                cv2.imshow("Video", execute(img, dst, t, connection))
